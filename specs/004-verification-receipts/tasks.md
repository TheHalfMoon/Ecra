...
- [x] **T052** Merge the exact expected implementation head by an allowed non-rebase method and require canonical-main ECR-004 + ECR-001 + ECR-002 workflows to succeed on the resulting canonical state.
- [x] **T053** Mark ECR-004 `CLOSED_CANONICAL` only after post-merge evidence; update roadmap/status/index/EXECUTION and re-evaluate dependency eligibility for ECR-005 and other slices from live canonical truth.
